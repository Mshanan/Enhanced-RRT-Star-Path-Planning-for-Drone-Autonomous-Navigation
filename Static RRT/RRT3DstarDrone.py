...
def pathPlanning(start, goal, obstacles):
    ...
    def ground_clearance(new_node, node_array, goal, max_extend_length):
        rows, _ = node_array.shape
        if rows > 2 and dist_to_goal(new_node, goal) >= max_extend_length + 20:
    ...
    def rewire(new_node, node_array, parent_array, cost_array, obstacles):
        ...
        for i in near_indices:
            ...
            if not collision(new_node, near_node, obstacles) and not max_turning_angle(nearest_node, new_node, parent_array) and not ground_clearance(new_node, node_array, goal, max_extend_length):
                new_cost = calc_new_cost(new_node, near_node, cost_array)
                if new_cost < cost_array[i]:
                    parent_array[i, :] = new_node
                    cost_array[i] = new_cost

    def plotting3D(start, goal, obstacles, path_array, node_array, parent_array, smoothed):
        ...
        for node, parent in zip(node_array, parent_array):
            ax.plot([node[0], parent[0]], [node[1], parent[1]], [node[2], parent[2]], "-g", linewidth=0.5)
        ax.scatter(node_array[:, 0], node_array[:, 1], node_array[:, 2], c='g', marker='o', s=1, label='RRT* Tree')

        # Plot the final path
        ax.plot(path_array[:, 0], path_array[:, 1], path_array[:, 2], "-r", label="RRT* Generated Path", linewidth=2)
        ax.scatter(path_array[:, 0], path_array[:, 1], path_array[:, 2], color="red")
        ax.plot(smoothed[:, 0], smoothed[:, 1], smoothed[:, 2], label="Smoothed RRT* Path", linewidth=2)
    ...
    # Set the boundary
    bounds_xy = np.array([-5, 5])  # Bounds for x and y axes
    bounds_z = np.array([0, 2])  # Bounds for z axis

    # RRT parameters
    max_extend_length = 1  # The maximal length extended from the nearest node toward the sampled node
    goal_sample_rate = 0.05  # Probability of the sample to be the goal: greediness of the planning toward goal
    max_iter = 250  # Maximal number of sample points
    search_radius = 4  # Radius for rewiring
    ...
    for i in range(max_iter):
        ...
        if not collision(nearest_node, new_node, obstacles) and not max_turning_angle(nearest_node, new_node, parent_array) and not ground_clearance(new_node, node_array, goal, max_extend_length):
        ...
        if dist_to_goal(node_array[-1], goal) < max_extend_length:
            if not collision(node_array[-1], goal, obstacles) and not max_turning_angle(nearest_node, new_node, parent_array):
    ...
    # spath = gradient_descent_smoothing(path_array)
    # plotting3D(start, goal, obstacles, path_array, node_array, parent_array, spath)
    print("Improved RRT")
...
# Kitchen counter
start = np.array([0, 0, 0])  # Start location in 3D
goal = np.array([5, 5, 0])  # Goal location in 3D
obstacles = [   
...
# Improved Final Maze
# max_extend_length = 20  # The maximal length extended from the nearest node toward the sampled node
# goal_sample_rate = 0.05  # Probability of the sample to be the goal: greediness of the planning toward goal
# max_iter = 6500  # Maximal number of sample points
# search_radius = 90  # Radius for rewiring
# start = np.array([-200, 200, 180])  # Start location in 3D
# goal = np.array([200, -200, 0])  # Goal location in 3D
# obstacles = [
#         np.array([0, 0, 100, 60, 60, 200]),
#         np.array([-200, 0, 100, 60, 60, 200]),
#         np.array([200, 0, 100, 60, 60, 200]),
#         np.array([0, 200, 100, 60, 60, 200]),
#         np.array([0, -200, 100, 60, 60, 200]),
#     ]  # cuboids parametrized by [x, y, z, dx, dy, dz]
# path, _ = pathPlanning(start, goal, obstacles)
