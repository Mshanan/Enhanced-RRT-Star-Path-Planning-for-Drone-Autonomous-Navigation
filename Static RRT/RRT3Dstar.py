...
def pathPlanning(start, goal, obstacles):
    ...
    def ground_clearance(new_node, node_array, goal, max_extend_length):
        rows, _ = node_array.shape
        if rows > 1 and dist_to_goal(new_node, goal) >= max_extend_length + 40:
    ...
    def plotting3D(start, goal, obstacles, path_array, node_array, parent_array):
        ...
        node_array = node_array/100
        parent_array= parent_array/100
        start = start/100
        goal = goal/100
        ...
        # Plot the final path
        path_array = path_array/100
        path_array = gradient_descent_smoothing(path_array)
    ...
    # Set the boundary
    bounds_xy = np.array([-350, 350])  # Bounds for x and y axes
    bounds_z = np.array([0, 200])  # Bounds for z axis

    # RRT parameters
    max_extend_length = 40  # The maximal length extended from the nearest node toward the sampled node
    goal_sample_rate = 0.05  # Probability of the sample to be the goal: greediness of the planning toward goal
    max_iter = 80000  # Maximal number of sample points
...
obstacles = [
    np.array([-100, -200, 100, 500, 50, 200]),
    np.array([100, 0, 100, 500, 50, 200]),
    np.array([-100, 200, 100, 500, 50, 200]),
]
