...
def pathPlanning(start, goal, obstacles):
    ...
    def max_turning_angle(nearest_node, new_node, parent_array):
        ...
        if rows >=2:
            ...
            if 0<=abs(theta_degrees)<=25 or 70<=abs(theta_degrees)<=110 or 160<=abs(theta_degrees)<=200 or 250<=abs(theta_degrees)<=290:
                return False
            else:
                return True
        else:
    ...
    def ground_clearance(new_node, node_array, goal, max_extend_length):
        rows, _ = node_array.shape
        if rows > 7 and dist_to_goal(new_node, goal) >= max_extend_length + 40:
    ...
    def plotting3D(start, goal, obstacles, path_array, node_array, parent_array):
        ...
        for node, parent in zip(node_array, parent_array):
            ax.plot([node[0], parent[0]], [node[1], parent[1]], [node[2], parent[2]], "-g", linewidth=1.5)
        ax.scatter(node_array[:, 0], node_array[:, 1], node_array[:, 2], c='g', marker='o', s=10, label='Nodes')

        # Plot the final path
        ax.plot(path_array[:, 0], path_array[:, 1], path_array[:, 2], "-m", label="Path", linewidth=2)
    ...
    # Set the boundary
    bounds_x = np.array([-100, 3300])  # Bounds for x 
...
# max_extend_length = 25  # The maximal length extended from the nearest node toward the sampled node
# goal_sample_rate = 0.05  # Probability of the sample to be the goal: greediness of the planning toward goal
# max_iter = 4000  # Maximal number of sample points
# search_radius = 50  # Radius for rewiring
# obstacles = [
