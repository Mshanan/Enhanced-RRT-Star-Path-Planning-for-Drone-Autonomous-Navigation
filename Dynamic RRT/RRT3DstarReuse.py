...
def pathPlanning(start, goal, obstacles, prev_path=None):
    ...
    def validate_remaining_path(prev_path, obstacles):
        ...
        valid_remaining_path = []
        local_goal = None
        local_start = None
        indices = []

        # Initialize the validation array with the same number of rows as prev_path
        validation = np.ones(prev_path.shape[0], dtype=int)  # 1 for valid, 0 for blocked

        # if there is a collision, put 0
        for i in range(prev_path.shape[0]-1):
            if collision(prev_path[i, :], prev_path[i+1, :], obstacles):
                validation[i] = 0

        # if node valid, store it
        for i in range(prev_path.shape[0]-1):
            if not collision(prev_path[i, :], prev_path[i+1, :], obstacles) :
                valid_remaining_path.append(prev_path[i, :])

        for i in range(prev_path.shape[0] - 3):
                if validation[i] == 1 and validation[i+1] == 1 and validation[i+2] == 1:
                    pass
                if validation[i] == 1 and validation[i+1] == 0 and validation[i+2] == 0 and validation[i+3] == 0:
                    local_goal = prev_path[i-2, :]
                    indices.append(i-2)
                if validation[i] == 0 and validation[i+1] == 0 and validation[i+2] == 0:
                    pass
                if validation[i] == 0 and validation[i+1] == 0 and validation[i+2] == 0 and validation[i+3] == 1:
                    local_start = prev_path[i+3, :]
                    indices.append(i+3)
                    
        if valid_remaining_path is not None:
            valid_remaining_path = np.array(valid_remaining_path)
        ...
        return local_start, local_goal, valid_remaining_path, indices     # Return the index of the last valid node
    ...
    if prev_path is None:
        local_start = start
        local_goal = goal
        valid_remaining_path = None
        indices = None
    elif prev_path is not None:
        local_start, local_goal, valid_remaining_path, indices = validate_remaining_path(prev_path, obstacles)
