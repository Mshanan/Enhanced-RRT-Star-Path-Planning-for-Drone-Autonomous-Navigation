...
from matplotlib.animation import FuncAnimation  # <- ADDED THIS IMPORT
...
def updatePositionIMU(x_imu, y_imu, dt):
    ...
    vx = drone.get_speed_x()
    vy = drone.get_speed_y()
    z = drone.get_height()*1.2
    yaw = drone.get_yaw()
...
def followPathOpenLoop(start, waypoints, prev_time, x_imu, y_imu):
    ...
    for wp in waypoints:
        if kp.getKey("b"):
            ...
            # drone.go_xyz_speed(int(x_dist), int(y_dist), int(z_dist), 50)

            # Update position
            current_time = time()
            dt = current_time - prev_time
            prev_time = current_time
            x, y, z = updatePositionModel(x, y, lr, fb, dt)
...
def update_plot():  # <- ADDED THIS FUNCTION
    ...
    model_path_line.set_data(path_x_cmd, path_y_cmd)
    model_path_line.set_3d_properties(path_z_cmd, zdir='z')
    
    # Plot the new path from the current start to the current goal
    imu_path_line.set_data(path_x_imu, path_y_imu)
    imu_path_line.set_3d_properties(path_z_imu, zdir='z')
    ...
    ax.plot(path_x_cmd, path_y_cmd, path_z_cmd, linewidth=2, c='g', label='Drone Model Path')
    ax.plot(path_x_imu, path_y_imu, path_z_imu, linewidth=2, c='b', label='Drone IMU Path')
    
    ax.scatter(0, 0, 0, c='g', marker='^', s=40, label='Start')
...
model_path_line, = ax.plot([], [], [], linewidth=2, c='g', label='Model Path')
imu_path_line, = ax.plot([], [], [], linewidth=2, c='b', label='IMU Path')

model_path_line.set_data([], [])
model_path_line.set_3d_properties([], zdir='z')
imu_path_line.set_data([], [])
imu_path_line.set_3d_properties([], zdir='z')
...
while True:
    ...
    if kp.getKey("f"):
        start = np.array([0, 0, drone.get_height()*1.2])  # get current position as start
